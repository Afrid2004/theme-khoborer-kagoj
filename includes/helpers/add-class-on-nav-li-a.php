<?php
  ...
  // adding class on footer_menu li
  function add_class_on_footer_menu_li($classes, $item, $args, $depth ){
    if($args->theme_location === 'footer-menu'){
      $classes[] = 'mb-3 mt-1 d-flex justify-content-center justify-content-md-start justify-content-lg-center';
    }
    return $classes;
  }
  add_filter('nav_menu_css_class', 'add_class_on_footer_menu_li', 10, 4);


  //adding class on footer_menu li a
  function add_class_on_footer_menu_li_a($atts, $item, $args, $depth){
    if($args->theme_location === 'footer-menu'){
      $exixting_class = isset($atts['class']) ? $atts['class'] : '';
      $atts['class'] = $exixting_class . ' text-decoration-none text-black';
    }
    return $atts;
  }
  add_filter('nav_menu_link_attributes', 'add_class_on_footer_menu_li_a', 10, 4);
